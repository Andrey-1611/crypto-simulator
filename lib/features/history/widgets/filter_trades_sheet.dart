...
class _FilterTradesSheetState extends ConsumerState<FilterTradesSheet> {
  ...
  Future<void> pickDateRange() async {
    final picked = await showDateRangePicker(
      ...
      initialDateRange: ref.read(filterTradesOnSheetProvider).dateRange,
      firstDate: DateTime(2020),
      lastDate: DateTime.now(),
    ...
    if (picked != null) {
      ref.read(filterTradesOnSheetProvider.notifier).state = ref
          .read(filterTradesOnSheetProvider)
          .copyWith(dateRange: picked);
    }
  }

  void apply() {
    ref.read(filterTradesProvider.notifier).state = ref.read(
      filterTradesOnSheetProvider,
    );
    context.pop();
  }
